...
fn read_to_end<T: Read>(
    bufreader: &mut BufReader<T>,
    buf: &mut Vec<u8>,
...
impl FileSystem {
    ...
    fn open_with_options(&self, path: &Path, open_options: &OpenOptions) -> FileSystemResult<File> {
        open_options
            .to_fs_openoptions()
            .open(path)
            .map_err(|io_error| FileSystemError::from(io_error))
    ...
    pub fn open(&self, path: &Path) -> FileSystemResult<BufReader<File>> {
        let buf = self.open_with_options(path, OpenOptions::new().set_read(true))?;
    ...
    pub fn write<T: Write>(
        &self,
        bufwriter: &mut BufWriter<T>,
        buf: &[u8],
    ) -> FileSystemResult<usize> {
        write(bufwriter, buf)
    }

    pub fn write_all<T: Write>(
        &self,
        bufwriter: &mut BufWriter<T>,
        buf: &[u8],
    ) -> FileSystemResult<()> {
    ...
    pub fn async_read<T: Read + Send>(
        ...
        thread_pool.install(|| read(bufreader, buf))
    }
    ...
    pub fn async_read_to_end<T: Read + Send>(
        ...
        thread_pool.install(|| read_to_end(bufreader, buf))
    }
    ...
    pub fn async_read_to_string<T: Read + Send>(
        ...
        thread_pool.install(|| read_to_string(bufreader, buf))
    }
    ...
    pub fn async_read_exact<T: Read + Send>(
        ...
        thread_pool.install(|| read_exact(bufreader, buf))
    }
    ...
    pub fn async_write<T: Write + Send>(
        ...
        thread_pool.install(|| write(bufwriter, buf))
    }
    ...
    pub fn async_write_all<T: Write + Send>(
        ...
        thread_pool.install(|| write_all(bufwriter, buf))
    }
}
...
mod filesystem_test {
    ...
    fn filesystem_io_operations() {
        let filesystem = FileSystem::new(GameInfos::new("test_filesystem_maskerad", "Malkaviel"))
            .expect("Couldn't create FS");

        let current_dir_dir_test = filesystem
        ...
        //user logs
        let user_log_dir_test = filesystem
        ...
        let async_log_dir_test = filesystem
            .construct_path_from_root(&RootDir::UserLogRoot, "async_dir/async_log_dir_test.txt")
            .expect("Could not create async_log_dir_test");
        {
            let mut log_bufwriter = filesystem
                .create(async_log_dir_test.as_path())
                .expect("Could not create the bufwriter");
        ...
        let mut bufreader_async = filesystem
            .open(async_log_dir_test.as_path())
            .expect("Could not create bufreader");
        ...
        filesystem
            .rmrf(current_dir_dir_test.as_path())
            .expect("Couldn't delete dir");
        assert!(!current_dir_dir_test.exists());
    }

    #[test]
    fn filesystem_read_dir() {
        let filesystem = FileSystem::new(GameInfos::new("test_filesystem_blacksmith", "Malkaviel"))
            .expect("Couldn't create FS");
        let src_dir = filesystem
