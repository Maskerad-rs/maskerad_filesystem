...
impl FileSystem {
    ...
    pub fn create(&self, path: &Path) -> FileSystemResult<BufWriter<File>> {
        let buf = self.open_with_options(path, OpenOptions::new().set_create(true).set_write(true).set_truncate(true))?;
    ...
    pub fn append(&self, path: &Path) -> FileSystemResult<BufWriter<File>> {
        let buf = self.open_with_options(path, OpenOptions::new().set_create(true).set_append(true).set_write(true))?;
        Ok(BufWriter::new(buf))
    }
    ...
    pub fn async_read<T: Read + Send>(&self, bufreader: &mut BufReader<T>, buf: &mut [u8], thread_pool: &ThreadPool) -> FileSystemResult<usize> {
        thread_pool.install(|| {
            read(bufreader, buf)
        })
    }
    pub fn async_read_to_end<T: Read + Send>(&self, bufreader: &mut BufReader<T>, buf: &mut Vec<u8>, thread_pool: &ThreadPool) -> FileSystemResult<usize> {
        thread_pool.install(|| {
            read_to_end(bufreader, buf)
        })
    }
    pub fn async_read_to_string<T: Read + Send>(&self, bufreader: &mut BufReader<T>, buf: &mut String, thread_pool: &ThreadPool) -> FileSystemResult<usize> {
        thread_pool.install(|| {
            read_to_string(bufreader, buf)
        })
    }
    pub fn async_read_exact<T: Read + Send>(&self, bufreader: &mut BufReader<T>, buf: &mut [u8], thread_pool: &ThreadPool) -> FileSystemResult<()> {
        thread_pool.install(|| {
            read_exact(bufreader, buf)
        })
    }
    ...
    pub fn async_write<T: Write + Send>(&self, bufwriter: &mut BufWriter<T>, buf: &[u8], thread_pool: &ThreadPool) -> FileSystemResult<usize> {
        thread_pool.install(|| {
            write(bufwriter, buf)
        })
    }
    pub fn async_write_all<T: Write + Send>(&self, bufwriter: &mut BufWriter<T>, buf: &[u8], thread_pool: &ThreadPool) -> FileSystemResult<()> {
        thread_pool.install(|| {
            write_all(bufwriter, buf)
        })
    }
...
mod filesystem_test {
    ...
    fn filesystem_io_operations() {
        ...
        //Metadata
        let file_metadata = async_log_dir_test.metadata().expect("Couldn't get metadata");
